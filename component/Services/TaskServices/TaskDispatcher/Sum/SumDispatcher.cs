...
{
    public static class SumDispatcher
    {
        private static string randomRange()
        {
            var rnd = new Random();
            var numbers = Enumerable.Range(0, 10000).Select(_ => rnd.Next(0, 1000)).ToList();
        ...
        public static async Task Start(MyTask task)
        {
            ...
            Logger.Log($"The Sum task is distributed among {count} nodes");
        }
    }
